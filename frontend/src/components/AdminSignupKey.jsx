...
const AdminSignupKey = () => {
  ...
  useEffect(() => {
    const fetchKeys = async () => {
      ...
      const res = await axios.get("https://goldenpluscaferms.onrender.com/api/auth/signup-keys", {
  ...
  const generateKey = async () => {
    ...
    const res = await axios.post(
      "https://goldenpluscaferms.onrender.com/api/auth/generate-key",
      {},
      {
        headers: { Authorization: `Bearer ${token}` },
      }
  ...
  const deleteKey = async (id) => {
    const token = localStorage.getItem("token");
    await axios.delete(`https://goldenpluscaferms.onrender.com/api/auth/signup-key/${id}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
