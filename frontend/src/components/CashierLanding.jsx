...
const CashierLanding = () => {
    ...
    if (digits.length >= 2) {
      ...
      try {
        ...
        const res = await axios.get(
          "https://goldenpluscaferms.onrender.com/api/auth/customers-search",
          {
            params: { q: digits },
            headers: { Authorization: `Bearer ${token}` }
          }
  ...
  const fetchDeliveryPlaces = async () => {
    try {
      const token = localStorage.getItem("token");
      const res = await axios.get("https://goldenpluscaferms.onrender.com/api/auth/delivery-charges", { // ✅ updated endpoint
        headers: { Authorization: `Bearer ${token}` }
      });
                    ...
                    onChange={async (selectedOption) => {
                      if (selectedOption && selectedOption.value === "__CREATE_NEW__") {
                        ...
                        try {
                          const token = localStorage.getItem("token");
                          const res = await axios.post(
                            "https://goldenpluscaferms.onrender.com/api/auth/delivery-charges",
